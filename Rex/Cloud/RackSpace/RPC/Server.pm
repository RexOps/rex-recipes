...
sub create {
  ...
  confess "error creating server. no id found."
    if ( !exists $ret->{server}->{id} );

  return $self->load( $ret->{server}->{id},
    admin_pass => $ret->{server}->{adminPass} );
}
...
sub load {
  ...
  my $req =
    $self->rs->_generate_request( method => HTTP_GET, action => "servers/$id" );
  my $ret = $self->rs->_do_request($req);
...
sub delete {
  ...
  my $req = $self->rs->_generate_request(
    method => HTTP_DELETE,
    action => "servers/" . $self->id
  );
  my $ret = $self->rs->_do_request($req);

  confess "error deleting server."
    if ( !ref $ret && !exists $ret->{ok} && $ret->{ok} != 1 );
}
...
sub set_admin_password {
  ...
  my $req = $self->rs->_generate_request(
    method => HTTP_POST,
    action => "servers/" . $self->id . "/action",
    post   => $post
  );
  my $ret = $self->rs->_do_request($req);

  confess "error setting admin password for server."
    if ( !ref $ret && !exists $ret->{ok} && $ret->{ok} != 1 );
}
...
sub reboot {
  ...
  confess "error rebooting server."
    if ( !ref $ret && !exists $ret->{ok} && $ret->{ok} != 1 );
}
