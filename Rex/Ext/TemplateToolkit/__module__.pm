...
sub validate_vars {
  ...
  foreach my $key (keys %$vars) {
      if ($key ~= /^[\._]/) {
	  warn "variable name '$key' considered private by Template Toolkit";
      } elsif ($key ~= /([^a-zA-Z0-9_])/) {
	  warn "variable name '$key' contains '$1' unsupported by Template Toolkit";
	  ...
	  warn "variable name '$key' clashes with reserved Template Toolkit word";
      }
  }
}
